...
def process(batch, args, pipeline):
    with torch.no_grad():
        ...
        # ================== 【新增：提取风格特征】 ==================
        style_embeds = None
        if style_encoder is not None and 'style_ref_image' in batch:
            style_tensor = batch['style_ref_image'].float().cuda()
            style_embeds = style_encoder(style_tensor)
        # ==========================================================
    ...
    with torch.no_grad():
        for i, batch in tqdm(enumerate(dataloader)):
            with torch.autocast("cuda"):
                result = process(batch, args, pipeline, style_encoder)
